...
def main():
    if len(sys.argv) != 3:
        print("Usage: extract_form_structure.py <pdf_path> <output_json_path>")
        sys.exit(1)
    pdf_path = sys.argv[1]
    ...
    print(f"Found: {len(structure['pages'])} pages, {len(structure['labels'])} labels, {len(structure['lines'])} lines, {len(structure['checkboxes'])} checkboxes")
